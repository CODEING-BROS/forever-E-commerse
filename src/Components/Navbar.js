  ...
  return (
    <div className='flex justify-between items-center py-5 font-medium '>
      <Link href="/" className='logo'>
        <Image className='w-36' src={assets.logo} alt="Logo" width={144} height={36} />
      </Link>
      ...
      <div className='hidden sm:flex gap-5 text-sm text-gray-700'>
        {nav.map((item) => {
          ...
          return (
            <Link
              key={item}
              className={`uppercase flex flex-col items-center gap-1 ${isActive ? 'active' : ''}`}
      ...
      <div className='flex items-center gap-6'>
        ...
        <div className='sm:hidden'>
          <Image onClick={() => setVisible(true)} className='w-5 cursor-pointer' src={assets.menu_icon} />
        </div>
      </div>
      ...
      <div className={`absolute top-0 right-0 bottom-0 overflow-hidden bg-white transition-all ${visible ? 'w-full' : 'w-0'}`}>
        <div className='flex flex-col text-gray-600'>
          <div onClick={() => setVisible(false)} className='p-3 flex items-center cursor-pointer gap-4'>
            <Image src={assets.dropdown_icon} className='w-[10px] h-4 rotate-180' />
            ...
            {nav.map((item) => {
              ...
              return (
                <Link
                  ...
                  key={item}
                  className={`hover:bg-gray-400 hover-font-bold hover:text-white py-2 pl-6 border-[1px] uppercase ${isActive ? 'active' : ''}`}
                  href={item === "Home" ? "/" : `/${item.toLowerCase()}`}
                >
                  {item}
                </Link>
